declare namespace Realm {
  ...
  type CollectionChangeCallback<T> = (
  ...
  interface FlexibleSyncConfiguration extends BaseSyncConfiguration {
    ...
    initialSubscriptions?: {
       ...
       */
      update: (
        subs: Realm.App.Sync.MutableSubscriptionSet,
        realm: Realm,
      ) => void;
      /**
...
type RealmListsRemappedModelPart<T> = {
  [K in ExtractPropertyNamesOfType<
    T,
    Realm.List<any>
  >]?: T[K] extends Realm.List<infer GT> ? Array<GT | Unmanaged<GT>> : never;
};
...
type RealmDictionaryRemappedModelPart<T> = {
  [K in ExtractPropertyNamesOfType<
    T,
    Realm.Dictionary
  >]?: T[K] extends Realm.Dictionary<infer ValueType>
    ? { [key: string]: ValueType }
    : never;
};
...
declare class Realm {
   ...
   * @returns {T | undefined}
   */
  objectForPrimaryKey<T extends Realm.Object>(
    type: { new (...arg: any[]): T },
    key: Realm.PrimaryKey,
  ): T | undefined;
   ...
   */
  objects<T extends Realm.Object>(type: {
    new (...arg: any[]): T;
  }): Realm.Results<T>;
  ...
  addListener(
    name: string,
    callback: (sender: Realm, event: 'change') => void,
  ): void;
  addListener(
    name: string,
    callback: (
